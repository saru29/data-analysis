...
#-------------------------
#Cycling Data
df = read_csv("test_set.csv", index_col='step', nrows=580)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
# define model
...
#-------------------------
# Run
print("Run 1 =  Test Train Set")
data = df.values
X, y = data[:, :-1], data[:, -1]
# define model
alpha_set, l1_ratio_set = CyclingElasticNetValues()
CyclingElasticNetModel(alpha_set, l1_ratio_set, X, y)
stop = input("Any key for next model/dataset: ")

# Different Data set: 400 rows of data, 1 second intervals
print("Run 2 =  1 second intervals")
df = read_csv("test_set_second.csv", index_col='step', nrows=135000)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
CyclingElasticNetModel(alpha_set, l1_ratio_set, X, y)
...
print("Run 3 =  3 second intervals")
df = read_csv("data_sample_3.csv", index_col='step', nrows=1740)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
CyclingElasticNetModel(alpha_set, l1_ratio_set, X, y)
...
stop = input("Target: ")
print("Run 4.2 =  watts per kilo and some added features - 1 minute intervals")
df = read_csv("wpkg_data.csv", index_col='step', nrows=80)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
...
# Session - 3 second intervals
print("Run 5 =  watts per kilo and some added features - 1 minute intervals")
df = read_csv("wpkg_data_2.csv", index_col='step', nrows=1740)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
CyclingElasticNetModel_V2(alpha_set, l1_ratio_set, X, y)

## Session - 1 second intervals
print("Run 6 =  6350 rows - watts per kilo - 1 sec intervals")
df = read_csv("wpkg_data_3.csv", index_col='step', nrows=6350)
#Get data and as
data = df.values
X, y = data[:, :-1], data[:, -1]
CyclingElasticNetModel_V2(alpha_set, l1_ratio_set, X, y)

## FINDINGS 1 minute intervals with added features provides a good basis for future models.
